...
| Ragged rows | A row with more/fewer fields than the header row shifts every later value into the wrong column, detected and auto-aligned automatically | No, always on |
| `@`-prefixed headers | Not fixed, just flagged. Data Merge treats a column named `@Photo` as an image field, not text | No, informational only |
...
to install. Drag a `.csv` onto the drop zone (or click to browse), review
the diagnostics, untick anything you don't want changed, then Copy or
Download the result. The preview grows to show every row, and the page
scrolls normally if the file's longer than one screen.
